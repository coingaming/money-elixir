defmodule Money.Constants do
  ...
  @currency_config Enum.reduce(@raw_config, %{}, fn {currency_code,
                                                     specs = %{
                                                       units: units,
                                                       code: code,
                                                       precision: precision
                                                    ...
                                                    acc
                                                    when is_atom(currency_code) and
                                                           is_binary(code) and
                                                           is_integer(precision) and
                                                           is_map(units) ->
                     Map.put(acc, Atom.to_string(currency_code), %{
                       specs
                       | units:
                           Enum.reduce(units, %{}, fn {currency_unit,
                                                       %{
                                                         code: unit_code,
                                                         name: unit_name,
                                                      ...
                                                      units_acc
                                                      when is_atom(currency_unit) and
                                                             ...
                                                             precision - shift >= 0 ->
                             Map.put(units_acc, Atom.to_string(currency_unit), unit_specs)
                           end)
                     })
                   end)
